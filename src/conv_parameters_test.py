...
class ModifiedResNet18(nn.Module):
    def __init__(self, input_channels=3, input_size=224):
        ...
        self.base_model.conv1 = nn.Conv2d(
            ...
            out_channels=max(1, self.base_model.conv1.out_channels // 64),
        ...
        self.base_model.bn1 = nn.BatchNorm2d(max(1, self.base_model.bn1.num_features // 64))

        # Scale down the layers
        def scale_down_layer(layer):
            for block in layer:
                block.conv1 = nn.Conv2d(
                    in_channels=max(1, block.conv1.in_channels // 64),
                    out_channels=max(1, block.conv1.out_channels // 64),
                    kernel_size=block.conv1.kernel_size,
                    stride=block.conv1.stride,
                    padding=block.conv1.padding,
                    bias=False
                )
                block.bn1 = nn.BatchNorm2d(max(1, block.bn1.num_features // 64))
                block.conv2 = nn.Conv2d(
                    in_channels=max(1, block.conv2.in_channels // 64),
                    out_channels=max(1, block.conv2.out_channels // 64),
                    kernel_size=block.conv2.kernel_size,
                    stride=block.conv2.stride,
                    padding=block.conv2.padding,
                    bias=False
                )
                block.bn2 = nn.BatchNorm2d(max(1, block.bn2.num_features // 64))
                if block.downsample is not None:
                    block.downsample[0] = nn.Conv2d(
                        in_channels=max(1, block.downsample[0].in_channels // 64),
                        out_channels=max(1, block.downsample[0].out_channels // 64),
                        kernel_size=block.downsample[0].kernel_size,
                        stride=block.downsample[0].stride,
                        padding=block.downsample[0].padding,
                        bias=False
                    )
                    block.downsample[1] = nn.BatchNorm2d(max(1, block.downsample[1].num_features // 64))
        ...
        self.base_model.fc = nn.Linear(8, 10)
...
input = np.int8(x*(2**(8-input_int_bits)))
